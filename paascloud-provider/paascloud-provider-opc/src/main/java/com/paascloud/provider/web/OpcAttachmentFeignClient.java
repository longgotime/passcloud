...
public class OpcAttachmentFeignClient extends BaseFeignClient implements OpcOssFeignApi {
    ...
    public Wrapper<List<OptUploadFileRespDto>> uploadFiles(List<OptUploadFileReqDto> fileReqDtoList) {
        ...
        try {
            for (OptUploadFileReqDto optUploadFileReqDto : fileReqDtoList) {
                OptUploadFileRespDto fileRespDto = opcAttachmentService.rpcUploadFile(optUploadFileReqDto);
                if (fileRespDto != null) {
                    result.add(fileRespDto);
                }
            }
        } catch (BusinessException ex) {
            logger.error("RPC上传附件信息, 出现异常={}", ex.getMessage(), ex);
