...
public class TpcMqTagController extends BaseController {
    ...
    @Resource
    private TpcMqTagFeignApi tpcMqTagFeignApi;
    ...
    public Wrapper<PageInfo<TpcMqTagVo>> queryTagListWithPage(@ApiParam(name = "tag", value = "角色信息") @RequestBody TpcMqTagQuery tpcMqTag) {

        logger.info("查询角色列表tpcMqTagQuery={}", tpcMqTag);

        return tpcMqTagFeignApi.queryTagListWithPage(tpcMqTag);
    }
    ...
    public Wrapper modifyProducerStatusById(@ApiParam(value = "修改tag状态") @RequestBody UpdateStatusDto updateStatusDto) {
        ...
        return tpcMqTagFeignApi.modifyProducerStatusById(updateStatusDto);
    }
    ...
    public Wrapper deleteTagById(@ApiParam(value = "Tag ID") @PathVariable Long id) {
        ...
        return tpcMqTagFeignApi.deleteTagById(id);
    }
}
