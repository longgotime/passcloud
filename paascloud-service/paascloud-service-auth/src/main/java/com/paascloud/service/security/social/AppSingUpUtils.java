...
import com.paascloud.service.security.AppSecretException;
...
public class AppSingUpUtils {
	...
	public void saveConnectionData(WebRequest request, ConnectionData connectionData) {
		redisTemplate.opsForValue().set(getKey(request), this.getPcConnectionData(connectionData), 10, TimeUnit.MINUTES);
	}
	...
	private PcConnectionData getPcConnectionData(ConnectionData connectionData) {
		PcConnectionData pcConnectionData = new PcConnectionData();
		BeanUtils.copyProperties(connectionData, pcConnectionData);
		return pcConnectionData;
	}

    private ConnectionData getConnectionData(PcConnectionData pcConnectionData) {
		String providerId = pcConnectionData.getProviderId();
		String providerUserId = pcConnectionData.getProviderUserId();
		String displayName = pcConnectionData.getDisplayName();
		String profileUrl = pcConnectionData.getProfileUrl();
		String imageUrl = pcConnectionData.getImageUrl();
		String accessToken = pcConnectionData.getAccessToken();
		java.lang.String secret = pcConnectionData.getSecret();
		java.lang.String refreshToken = pcConnectionData.getRefreshToken();
		Long expireTime = pcConnectionData.getExpireTime();
